...
export class Calculation {
  ...
  calculateItemMoney(action: ActionType, item: number) {
    if (action === "REBUY") {
      const rebuyMoney = this.config.rebuy
        ? this.config.rebuy * item
        : 40 * item;
    ...
    if (action === "ADDON") {
      const addOnMoney = this.config.addOn
        ? this.config.addOn * item
        : 40 * item;
    ...
    if (action === "BUYIN") {
      const buyInMoney = this.config.buyIn
        ? this.config.buyIn * item
        : 40 * item;
